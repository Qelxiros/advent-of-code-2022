...
//        String[] relevantValves = new String[]{"BB","CC","DD","EE","HH","JJ"};
        ...
        ElephantConfig start = new ElephantConfig(new HashSet<>(), valves.get("AA"), valves.get("AA"), 0, 0, 0);
        PriorityQueue<ElephantConfig> queue = new PriorityQueue<>();
        queue.add(start);
        int most = 0;
        while (!queue.isEmpty()) {
            ElephantConfig current = queue.poll();
            List<ElephantConfig> neighbors = current.getNeighbors(valves, relevantValves, dists, distFromAA);
            if (neighbors.size() == 0) {
                if (current.pressureReleased > most) {
                    most = current.pressureReleased;
